...
case class TLToAHBNode(supportHints: Boolean)(implicit valName: ValName) extends MixedAdapterNode(TLImp, AHBImpMaster)(
  dFn = { cp =>
    val masters = cp.clients.map { case c => AHBMasterParameters(name = c.name, nodePath = c.nodePath) }
    AHBMasterPortParameters(masters, userFields = cp.userFields.filter(!_.isInstanceOf[AMBAProtField]))
  },
  uFn = { case sp =>
...
class AHBControlBundle(params: TLEdge) extends GenericParameterizedBundle(params)
{
  ...
  val hauser = BundleMap(params.bundle.userFields.filter(_.key.isRequest))
}
