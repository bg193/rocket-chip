...
case object AXI4FragLast extends ControlEchoKey[Bool]("real_last")
...
class AXI4Fragmenter()(implicit p: Parameters) extends LazyModule
{
  ...
  val node = AXI4AdapterNode(
    masterFn = { mp => mp.copy(masters = mp.masters.map(m => mapMaster(m)), userFields = AXI4FragLastField() +: mp.userFields) },
  ...
  lazy val module = new LazyModuleImp(this) {
    (node.in zip node.out) foreach { case ((in, edgeIn), (out, edgeOut)) =>
      ...
      out.ar :<>: in_ar
      out.ar.bits.user(AXI4FragLast) := ar_last
      ...
      out.aw.bits.user(AXI4FragLast) := aw_last
      ...
      // R flow control
      val r_last = out.r.bits.user(AXI4FragLast)
      in.r :<> out.r
      in.r.bits.last := out.r.bits.last && r_last

      // B flow control
      val b_last = out.b.bits.user(AXI4FragLast)
      in.b :<> out.b
