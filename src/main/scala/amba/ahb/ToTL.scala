...
case class AHBToTLNode()(implicit valName: ValName) extends MixedAdapterNode(AHBImpSlave, TLImp)(
  dFn = { case mp =>
    TLClientPortParameters(
      ...
      userFields = AMBAProtField() +: mp.userFields)
  },
...
class AHBToTL()(implicit p: Parameters) extends LazyModule
{
  ...
  lazy val module = new LazyModuleImp(this) {
    (node.in zip node.out) foreach { case ((in, edgeIn), (out, edgeOut)) =>
      ...
      val d_user  = Reg(BundleMap(edgeOut.bundle.userFields.filter(_.key.isRequest)))
      ...
      when (a_accept) {
        ...
        when (a_first) {
          ...
          d_user  :<= in.hauser
          d_user(AMBAProt).fetch      := !in.hprot(0)
          d_user(AMBAProt).privileged :=  in.hprot(1)
          d_user(AMBAProt).bufferable :=  in.hprot(2)
          d_user(AMBAProt).cacheable  :=  in.hprot(3)
          d_user(AMBAProt).secure     := false.B
          d_user(AMBAProt).modifiable :=  in.hprot(3)
        }
      }
